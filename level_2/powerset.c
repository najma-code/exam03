...
void	print_subset(int *arr, int *used, int len)
{
	int first = 1;
...
void	backtrack(int *arr, int *used, int len, int idx, int sum, int target)
{
	if (idx == len)		// BASE CASE
	{
		if (sum == target)
			print_subset(arr, used, len);
		return ;
	}
	if (sum > target)	// PRUNING
		return ;
	used[idx] = 1;
	backtrack(arr, used, len, idx + 1, sum + arr[idx], target);	// sum + arr[idx]: the new sum if we include arr[idx] in the current subset
	used[idx] = 0;
	backtrack(arr, used, len, idx + 1, sum, target);			// Skip arr[idx] and continue
}
...
int	main(int argc, char **argv)
{
	...
	for (int i = 0; i < len; i++)
		arr[i] = atoi(argv[i + 2]);
	backtrack(arr, used, len, 0, 0, target);
	free(arr);
	free(used);
	return (0);
}
